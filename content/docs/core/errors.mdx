...
The value could not be coerced to the target type. Used by `errCoerce()` in validators.
...
```ts
import { errMissing, errType, errInvalid, errCoerce, errWrap } from "@ctroenv/core"
...
errCoerce("COUNT", "abc", "number") // code: "coercion_failed"
